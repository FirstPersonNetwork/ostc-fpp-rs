...
/// Defined Message Types for OSTC
    ...
    fn from(value: MessageType) -> Self {
        match value {
            MessageType::RelationshipRequest => {
                "https://linuxfoundation.org/ostc/1.0/relationship-request".to_string()
            }
            MessageType::RelationshipRequestRejected => {
                "https://linuxfoundation.org/ostc/1.0/relationship-request-reject".to_string()
            }
            MessageType::RelationshipRequestAccepted => {
                "https://linuxfoundation.org/ostc/1.0/relationship-request-accept".to_string()
            }
            MessageType::RelationshipRequestFinalize => {
                "https://linuxfoundation.org/ostc/1.0/relationship-request-finalize".to_string()
            }
...
impl TryFrom<&str> for MessageType {
    type Error = OSTCError;

    fn try_from(value: &str) -> Result<Self, Self::Error> {
        match value {
            "https://linuxfoundation.org/ostc/1.0/relationship-request" => {
                Ok(MessageType::RelationshipRequest)
            }
            "https://linuxfoundation.org/ostc/1.0/relationship-request-reject" => {
                Ok(MessageType::RelationshipRequestRejected)
            }
            "https://linuxfoundation.org/ostc/1.0/relationship-request-accept" => {
                Ok(MessageType::RelationshipRequestAccepted)
            }
            "https://linuxfoundation.org/ostc/1.0/relationship-request-finalize" => {
            ...
            _ => Err(OSTCError::InvalidMessage(value.to_string())),
        }
    }
}
