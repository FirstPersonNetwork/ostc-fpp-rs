...
impl Contacts {
    ...
    pub async fn add_contact(
            ...
            && self.aliases.contains_key(alias)
        {
            ...
            return Err(OSTCError::Contact(format!(
...
impl ProtectedConfig {
    ...
    pub fn save(&self, seed_bytes: &SecretVec<u8>) -> Result<String, OSTCError> {
    ...
    pub fn load(seed_bytes: &SecretVec<u8>, input: &str) -> Result<ProtectedConfig, OSTCError> {
    ...
    pub fn get_seed(bip32: &ExtendedSigningKey, path: &str) -> Result<SecretVec<u8>, OSTCError> {
        Ok(SecretVec::new(
            bip32
                .derive(&path.parse::<DerivationPath>().map_err(|e| {
                    OSTCError::BIP32(format!("Couldn't parse derivation path ({}): {}", path, e))
                })?)
                .map_err(|e| {
                    OSTCError::BIP32(format!(
