...
use crate::{config::PersonaDIDKeys, errors::OSTCError};
...
pub fn create_initial_webvh_did(
...
) -> Result<(String, Document), OSTCError> {
    let did_url = WebVHURL::parse_url(&Url::parse(raw_url).map_err(|e| {
        OSTCError::WebVH(DIDWebVHError::ValidationError(format!(
    ...
    let mut did_document = Document::new(&did_url.to_string())
        .map_err(|e| OSTCError::Config(format!("Invalid DID URL: {e}")))?;
    ...
    let next_update_key = bip32_root
        ...
        .map_err(|e| {
            OSTCError::BIP32(format!("Failed to create an Ed25519 next_update key. {e}"))
        })?;
