/*! Contains the OSTC CLI Tool Configuration
*
...
/// Configuration information for ostc tool
...
impl Config {
    ...
    pub fn load_step1(profile: &str) -> Result<PublicConfig, OSTCError> {
    ...
    ///
    pub async fn get_persona_keys(&self, tdk: &TDK) -> Result<PersonaDIDKeys, OSTCError> {
        let signing = if let Some(signing) = self.persona_did.document.assertion_method.first() {
            ...
            let Some(ki) = self.key_info.get(signing.get_id()) else {
                return Err(OSTCError::Config(format!(
                    "Couldn't find key info in ostc Config for ({})",
                    signing.get_id()
                )));
            };
            KeyInfo {
