...
fn get_config_path(profile: &str) -> Result<String, OSTCError> {
...
impl PublicConfig {
    ...
    /// Uses the default CONFIG_PATH const or ENV Variable OSTC_CONFIG_PATH
    pub fn save(
        &self,
        profile: &str,
    ...
    /// Loads from disk the public information for OSTC to unlock it's secrets from the OS Secure
    /// Store
    pub fn load(profile: &str) -> Result<Self, OSTCError> {
        let cfg_path = get_config_path(profile)?;
        let path = Path::new(&cfg_path);

        let file =
            fs::File::open(path).map_err(|e| OSTCError::ConfigNotFound(cfg_path.to_string(), e))?;
