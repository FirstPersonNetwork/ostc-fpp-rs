...
use crate::{KeyPurpose, errors::OSTCError};
use affinidi_tdk::{
...
pub fn get_bip32_root(seed: &[u8]) -> Result<ExtendedSigningKey, OSTCError> {
    ExtendedSigningKey::from_seed(seed)
        .map_err(|e| OSTCError::BIP32(format!("Couldn't create BIP32 Master Key from seed: {}", e)))
}

pub trait Bip32Extension {
    fn get_secret_from_path(&self, path: &str, kp: KeyPurpose) -> Result<Secret, OSTCError>;
}
        ...
        let secret = match kp {
            ...
            KeyPurpose::Encryption => {
                ...
                Secret::generate_x25519(None, Some(&x25519_seed)).map_err(|e| {
                    OSTCError::Secret(format!("Failed to create derived encryption key: {}", e))
                })?
            }
            _ => {
                return Err(OSTCError::Secret(format!(
