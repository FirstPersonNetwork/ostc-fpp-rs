...
pub fn token_encrypt(
...
) -> Result<(Vec<u8>, Vec<u8>), OSTCError> {
    let mut card = open_card(token_id)
        .map_err(|e| OSTCError::Token(format!("Couldn't use hardware token ({token_id}): {e}")))?;
    let mut card = card.transaction().map_err(|e| {
        OSTCError::Token(format!(
    ...
    let cs = CardSlot::init_from_card(&mut card, KeyType::Decryption, touch_prompt)
        .map_err(|e| OSTCError::Token(format!("Couldn't init decryption key from token: {e}")))?;
    ...
    let esk = pk
        ...
        .map_err(|e| OSTCError::Encrypt(format!("Couldn't encrypt config data: {e}")))?;
    ...
    Ok((
        esk.to_bytes().map_err(|e| {
            OSTCError::Encrypt(format!("Couldn't convert encrypted ESK to bytes: {e}"))
        })?,
        encrypted,
    ))
}
    ...
    T: TokenInteractions + Send + Sync,
{
    ...
    card.verify_user_pin(user_pin.to_owned())
        .map_err(|_| OSTCError::TokenBadPin)?;
    card.to_user_card(None)
        .map_err(|e| OSTCError::Token(format!("Couldn't unlock user mode on token: {e}")))?;
    ...
    let pk_esk = PkeskBytes::try_from_reader(&PublicKeyAlgorithm::ECDH, 6, raw_br)
        .map_err(|e| OSTCError::Decrypt(format!("Couldn't convert public-key ESK. Reason: {e}")))?;
    let (decrypted_esk, _) = cs
        .decrypt(&pk_esk)
        .map_err(|e| OSTCError::Token(format!("Couldn't decrypt data, reason: {e}")))?;
    ...
    if decrypted_esk.len() != 32 {
        ...
        return Err(OSTCError::Decrypt(format!(
