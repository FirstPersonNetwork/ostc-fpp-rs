...
use anyhow::Result;
#[cfg(feature = "openpgp-card")]
use ostc::openpgp_card::{factory_reset, get_cards};
use ostc::{
                    ...
                    Action::SetupCompleted(setup_flow) => {
                        ...
                        match Config::create(&state.setup, &setup_flow, tdk, &self.profile).await {
                            ...
                            Err(e) => {
                                ...
                                state.setup.final_page.messages.push(MessageType::Error(format!("Couldn't create OSTC configuration. Reason: {e}")));
                            }
                        }
                    },
                    _ => {}
