...
/// Checks if another instance of ostc is running for the same profile
        ...
        Ok(exists) => {
            if exists {
                ...
                if system.process(Pid::from_str(&pid)?).is_some() {
                    eprintln!(
                        "{}{}{} {}",
                        style("ERROR: Another instance of ostc is running for this profile (")
                            .color256(CLI_RED),
                        style(profile).color256(CLI_PURPLE),
                        style(")!").color256(CLI_RED),
                        style(
                            "Only a single instance of ostc can run for a given profile at a time!"
                        )
                        .color256(CLI_ORANGE)
                    );
                    bail!("Duplicate ostc instance running");
                }
            }
        }
        Err(e) => {
...
fn get_lock_file(profile: &str) -> Result<String> {
    let path = if let Ok(config_path) = env::var("OSTC_CONFIG_PATH") {
...
async fn main() -> Result<()> {
    ...
    let profile = if let Ok(env_profile) = env::var("OSTC_CONFIG_PROFILE") {
        ...
        if cli_profile != "default" && cli_profile != env_profile {
            println!("{}", 
                style("WARNING: Using both ENV OSTC_CONFIG_PROFILE and CLI profile! These do not match!").color256(CLI_ORANGE)
            );
            println!(
                "{} {}",
            ...
            println!(
                ...
                style(") from OSTC_CONFIG_PROFILE ENV variable").color256(CLI_BLUE)
            );
            env_profile
        }
    } else {
    ...
    if let StartingMode::NotSet = starting_mode {
        match load(&profile).await {
            ...
            Err(ostc::errors::OSTCError::ConfigNotFound(_, _)) => {
    ...
    // OSTC must be in either setup or main state
    if let StartingMode::NotSet = starting_mode {
...
/// Loads ostc with Trust Development Kit (TDK) and Config
/// This does not need to be called for setup!
async fn load(profile: &str) -> Result<(TDK, Config), OSTCError> {
    // Instantiate the TDK
    let mut tdk = TDK::new(
    ...
    let a = {
        use ostc::config::TokenInteractions;
        ...
        .await
        {
            ...
            Err(OSTCError::TokenBadPin) => {
                if loop_count == 0 {
                    ...
                    return Err(OSTCError::TokenBadPin);
                } else {
                    println!(
                        "{}",
