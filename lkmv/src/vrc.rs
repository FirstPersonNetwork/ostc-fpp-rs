...
pub struct Vrcs {
    ...
    vrcs: HashMap<Rc<String>, HashMap<Rc<String>, Rc<DTGCredential>>>,
}

impl Vrcs {
    /// Get all VRC Values
    pub fn values(&self) -> Values<'_, Rc<String>, HashMap<Rc<String>, Rc<DTGCredential>>> {
        self.vrcs.values()
    }

    /// Get all the remote P-DID keys that exist with a VRC
    pub fn keys(&self) -> Keys<'_, Rc<String>, HashMap<Rc<String>, Rc<DTGCredential>>> {
        self.vrcs.keys()
    }

    /// Get all VRCs for a specific P-DID
    pub fn get(&self, id: &Rc<String>) -> Option<&HashMap<Rc<String>, Rc<DTGCredential>>> {
        self.vrcs.get(id)
    }

    /// Insert a new VRC for the given remote P-DID
    pub fn insert(&mut self, remote_p_did: &Rc<String>, vrc: Rc<DTGCredential>) {
        let hash = Rc::new(vrc.proof_value().unwrap().to_string());
...
pub trait DtgCredentialMessage {
    ...
    fn message(&self, from: &str, to: &str, thid: Option<&str>) -> Result<Message, LKMVError>;
}
