...
impl Contacts {
    ...
    pub async fn add_contact(
            ...
            && self.aliases.contains_key(alias)
        {
            ...
            return Err(LKMVError::Contact(format!(
    ...
    pub fn remove_contact(&mut self, logs: &mut Logs, id: &str) -> Option<Rc<Contact>> {
...
impl PrivateConfig {
    ...
    pub fn get_seed(bip32: &ExtendedSigningKey, path: &str) -> Result<SecretVec<u8>, LKMVError> {
        Ok(SecretVec::new(
            bip32
                .derive(&path.parse::<DerivationPath>().map_err(|e| {
                    LKMVError::BIP32(format!("Couldn't parse derivation path ({}): {}", path, e))
                })?)
                .map_err(|e| {
