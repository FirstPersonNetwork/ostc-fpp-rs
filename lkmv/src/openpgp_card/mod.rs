...
pub fn get_cards() -> Result<Vec<Arc<Mutex<Card<Open>>>>, LKMVError> {
    let mut cards = vec![];

    for backend in PcscBackend::cards(None)
        .map_err(|e| LKMVError::Token(format!("Couldn't get list of tokens. Reason: {e}")))?
    {
        let card =
            Card::<Open>::new(backend.map_err(|e| {
                LKMVError::Token(format!("Couldn't get card backend. Reason: {e}"))
            })?)
            .map_err(|e| LKMVError::Token(format!("Couldn't open card. Reason: {e}")))?;
