...
async fn remote_ping(tdk: &TDK, config: &mut Config, remote: &str) -> Result<()> {
    ...
    println!("{}", style("✅ Ping Successfully sent... Run ostc tasks interactive to check for pong response. NOTE: The remote recipient needs to check their messages first!").color256(CLI_GREEN));
