...
pub async fn print_status(term: &Term, tdk: &mut TDK, profile: &str) {
    println!(
        "{}",
        style("First Person Protocol (OSTC) tool").color256(CLI_BLUE)
    );
    println!(
        "{}",
    ...
    println!(
        "{} {}",
        style("ostc version:").color256(CLI_BLUE),
    ...
    .await
    {
        Ok(cfg) => {
            println!(
                "{} {}",
                style("ostc secured configuration:").color256(CLI_BLUE),
...
fn feature_flags() {
    print!("{} ", style("ostc enabled features:").color256(CLI_BLUE),);
...
fn openpgp_cards_status() -> Result<()> {
    ...
    #[cfg(feature = "openpgp-card")]
    {
        use ostc::openpgp_card::get_cards;
