/*! Handles the setup of the ostc CLI tool
*/
...
pub async fn cli_setup(term: &Term, profile: &str) -> Result<()> {
    println!(
        "{}",
        style("Initial setup of the ostc tool").color256(CLI_GREEN)
    );
    println!();
        ...
        public: PublicConfig {
            ...
            logs: Logs {
                messages: VecDeque::from([LogMessage {
                    ...
                    message: "Initial ostc setup completed".to_string(),
                }]),
                ..Default::default()
            },
            friendly_name,
    ...
    println!(
        ...
        style("ostc status").color256(CLI_GREEN),
...
fn change_mediator() -> String {
    println!();
    println!("{}", style("ostc utilizes DIDComm protocol to communicate. ostc requires the use of a DIDComm Mediator to store and forward messages between parties privately and securely").color256(CLI_BLUE));
    println!(
        "{} {}",
...
fn change_lf_did() -> String {
    println!();
    println!("{}", style("ostc interacts with the Linux Foundation , Linux Foundation is represented by a well-known DID. Do not change the following unless you know what you are doing!").color256(CLI_BLUE));
    println!(
        "{} {}",
